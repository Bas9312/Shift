...
class TerminalDeepDiveCommands(
    ...
    fun handleDeepDiveStartCommand() {
        ...
        val deepDiveText = """
            ...
            Погружение идёт — делай то, за чем нырял.

            Когда закончишь, мастер назовёт ГЛУБИНУ (число от 1 до 5) — по тому,
            что ты успел там сделать. С этим числом и выходишь:
            DEEP_DIVE.END <глубина>

            Без числа от мастера выхода нет.
        """.trimIndent()
        ...
        activity.sendTextToMg(
            "Ушёл в глубокое погружение. Когда закончит — назовите ему глубину (1-5) " +
                "по тому, что он там делал: с этим числом он выходит"
        )
