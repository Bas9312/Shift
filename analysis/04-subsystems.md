...
> ℹ️ **Аудит подсистем от 2026-07-22, размеченный по ходу работ.** Находки помечены
> прямо здесь (ИСПРАВЛЕНО / проверено-не-воспроизводится), но разметка отставала от кода
> и ссылки `файл:строка` устарели. Сводное текущее состояние — в [11-status.md](11-status.md).
