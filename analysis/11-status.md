...
> **State after the 2026-09-08 emulator session: A8, A9 and A10 are closed.** What is left in
> this section is A1 (the one that needs a real phone, not an emulator) and the four small
> checks A2–A7 that mutate production game state. A9 and A10 were closed by writing throwaway
> data into the live database with the owner's explicit permission, taking a backup first
> (`SERVER/_backups/bas931wn_inst1-20260908-163458.sql.gz`) and deleting the test rows
> afterwards — that is the pattern to reuse, not an exception.
...
| A9-note | Two notifications per chain point | Not a defect, but worth a decision before the game: entering a chain point raises **two** notifications back to back — the ordinary point notification built from `textToShowOnEnter` («📍 QA0908 старт») and the chase event one («📍 След взят»). Observed on every chain point. Fine if intended; noisy if not. |
