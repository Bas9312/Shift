...
> **State after the 2026-09-08 emulator session: A2, A5, A6, A8, A9 and A10 are closed.**
> Three items remain, and none of them is a matter of effort:
> **A1** needs a real phone lying still with the screen off — an emulator cannot answer it, and
> it is still the single highest-value open item before the game.
> **A4** needs a human in the emulator's Extended Controls (or a real camera); there is no `adb`
> path to it at all.
> **A7** is one button press away — everything up to the send was verified, but synthetic taps
> do not land on that view.
> **A3** was not attempted: it fires real 24-hour effects on the live account, and unlike the
> others it is not obviously safe to undo.
>
> Six of them were closed by writing throwaway data into the live database with the owner's
> explicit permission, taking a backup first
> (`SERVER/_backups/bas931wn_inst1-20260908-163458.sql.gz`) and deleting every test row
> afterwards — chain, quest, links, progress, messages, and the temporarily-raised noise all
> verified back to zero. That is the pattern to reuse, not an exception.
...
| ~~A2~~ | ~~Terminal noise visual effects live~~ | **Done 2026-09-08, all four tiers seen on screen.** Personal noise was set directly in `noisemancy_local` and restored to `0.0` afterwards. Note the scale: the server halves the stored value before the client sees it, so `local_noise = 5.0` renders as UI 2.5. Level 2 → grain over the terminal; level 3 → heavier grain (glitch and vibration also fire, both invisible in a screenshot); level 4 → the whole screen goes red (`showRedScrim`); level 5 → red plus the horned silhouette of `demonJumpScare`. |
...
| A4 | Successful QR/barcode scan path in `AuraScannerActivity` / `ArtifactScannerActivity` | **Still blocked, and structurally so.** The emulator's back camera is `virtualscene`, and putting a test QR in front of it means swapping the scene poster through Extended Controls — a GUI dialog with no `adb` equivalent. Not a matter of effort: an automated session cannot do this one at all. Cancel and permission-denial paths *are* verified. Needs either a real phone or a human clicking through Extended Controls. |
| ~~A5~~ | ~~Live check of the session-55 `NewMessagesChecker` fix~~ | **Done 2026-09-08.** Two unread messages from different senders (`MG_ISA`, `MG_ANTE`) were inserted and the notification read «У вас 2 новых сообщений» — the counter is right. The wording was not: Russian needs «сообщения» for 2–4. Fixed the same session with a `plurals` resource and re-verified with three messages → «У вас 3 новых сообщения». Test rows deleted. |
| ~~A6~~ | ~~MG-side branch of `UserRoles.isMg`~~ | **Done 2026-09-08**, logged in as `MG_Bas` and opened a chat that happens to contain all three cases at once. `MessagesAdapter` decided: the player's own message → not highlighted, shown under the interlocutor's name; a message from **another** master (`MG_ANTE`) → `isFromMG=true`, highlighted as own, labelled with the raw id; own message → «Вы». That is exactly what the branch intends. The MG chat list also renders with per-chat unread badges, which incidentally re-confirms the session-57 access fix. |
| A7 | **Live check of the P3 attachment streaming fix** | **Attempted 2026-09-08, got most of the way and stopped.** A 15 MB PNG was pushed to the emulator, the attach button opened the system photo picker, the file was selected and accepted back into the chat screen — so the picking half works. The send itself could not be triggered: `input tap` on the send button produced no reaction and no log line, twice, with the button provably enabled for a player (`updateSendButtonState`). Synthetic taps on that particular view do not land; a real finger would. **What is left for whoever has the phone:** press send, confirm the photo arrives in the MG chat intact, and check `run-as bas.app.shift ls cache/` has no leftover `attach_upload_*` files. Everything up to the press is verified. |
...
- ~~**Потолок глобального шума разъехался между копиями**~~ — **выровнен 2026-09-09** (Wave 35): `api.php` теперь тоже ограничивает глобалку `NOISE_GMAX`, как это делали `service.php` и `siteNoisemancy`. Сами три копии формулы никуда не делись — см. следующий пункт.
- **Начисление шума размножено на сервере в трёх копиях** (найдено
  ...
  считают шум по одной и той же формуле, скопированной трижды. Разъехавшийся потолок глобалки
  выровнен (Wave 35), но **сами копии остались**: любая будущая правка формулы должна вноситься
  в три места, иначе они снова разойдутся. Свести их в одну мешает то, что третья живёт на слое
  БД движка, а не на PDO — нужен либо перенос, либо параметризация, как сделано с делением.
...
Хранится удвоенным, чтобы **целые цены команд давали полушаговую точность**: «цена 1» —
это полуровня, «цена 2» — уровень. Иначе в ценах пришлось бы писать 0.5 и 1.0. Из той же
логики круглые константы: софткап `NOISE_SOFTCAP_LSOFT = 6.0` — это ровно уровень 3 в UI,
то есть перегиб включается там, где начинаются эффекты; крон `local_down = 2.0` снимает
ровно один уровень; `NOISE_LMAX = 10` — это пять уровней по два.

**Ловушка — асимметрия знака** (`api.php:261`, помечена в коде как «*** ключевая правка»):
отрицательная дельта умножается на 2 и означает уровни UI (`-2` = минус два уровня), а
положительная уходит в сырую шкалу как есть и означает полууровни (`+2` = плюс один
уровень). Это не замысел, а поздняя заплатка: чинили `USER.FORMAT` (−10), который не
сбрасывал шум полностью, и удвоили только минус — на плюсе к тому моменту был подогнан
баланс (множитель активности, софткап, коэффициент 1.4). Отсюда ощущение, что цифры не
сходятся: поле `delta` значит разное в зависимости от знака.

Выправлять сейчас нельзя — честный плюс вдвое усилит весь рост шума. Правильный момент —
перенос цен команд в серверный справочник (второй этап работ по шумомантии): задать их
сразу в UI-уровнях с дробями и убрать асимметрию, оставив одну шкалу на всё.
