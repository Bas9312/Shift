...
> **Ни один пункт больше не заблокирован на других людях** (состояние на 2026-09-08):
> **#3 снят совсем** — Женя делает всё на стороне сайта, поддержка на мобилке не нужна;
> **#25 отложен** решением владельца, ждать Колю не нужно, просто не делаем;
> **вопросы к Тари закрыты** — ответы нашлись в серверном коде и в живой БД, см. раздел 5.
> **#7** отложен решением владельца.
> **#15 закрыт целиком (владелец, 2026-09-08):** геймдизайнерские вопросы (личный или
> командный прогресс, даёт ли что-то финиш, можно ли перепрыгнуть точку) сняты — клиенту это
> безразлично, он показывает факт финиша, и этого достаточно. Ждать Лёшу больше не нужно,
> механика считается готовой.
...
### ~~#3 (жёлтый) — Шумомантия ломает сайт~~ — СНЯТ 2026-09-08

Женя сказал, что сделает всё на стороне сайта сам, и поддержка со стороны мобильного
клиента не нужна. Пункт закрыт, ничего не ждём и не делаем.

Что предполагалось раньше (оставлено только чтобы никто не «восстановил» задачу): команды
терминала вида `SITE.KARMA`, `SITE.POST.ANON`, `SITE.POST.AS` с высоким `noiseIncrease`,
по образцу `TerminalProxyCommands`, после того как Женя даст эндпоинты. Эндпоинтов не будет —
точнее, они будут, но дёргать их из приложения не потребуется.
...
- **#25 лента игротехнических сообщений** — **отложено решением владельца 2026-09-08.**
  Не «ждём Колю», а сознательно не делаем в этот выезд. Если когда-нибудь вернёмся: клиенту
  нужен режим «сообщение без возможности ответа», флага в `Message` нет, тащить придётся
  через `tags`, и первым делом надо будет договориться, какой именно тег ставит рассылка.
- ~~**#3 шумомантия ломает сайт**~~ — снят, Женя делает это на сайте без участия клиента.
...
просто не внесли в док.

**Ответ найден в коде 2026-09-08, спрашивать Тари не нужно: точку с `createdAt` в будущем
сервер прячет.** `GET /points` строит выборку от
`$baseWhere = "(expireAt > NOW() OR expireAt IS NULL) AND (createdAt <= NOW()) AND …"`
(`api_geo/api.php:283`), и этот `$baseWhere` одинаково применяется к ветке МГ и к ветке
игрока. То есть дата в будущем работает как отложенная публикация, и **риска по #13 нет**.

Два следствия, которые стоит помнить:
1. **МГ тоже не увидит такую точку на карте** до наступления времени — он не исключение из
   фильтра. В веб-панели она видна всегда, потому что панель читает таблицу напрямую.
2. Живая проверка: точек с `createdAt` в будущем в базе сейчас **ноль**, так что механика
   ни на чём сейчас не держится и сломать её нечем.
...
отменённому варианту.

**Проверено на живом сервере и в живой БД 2026-09-08 — док мёртвый, спрашивать Тари нечего.**
Таблиц `chats` и `master_subscriptions`, на которых держится вся схема из дока, в базе
**не существует** (`SHOW TABLES` — пусто по обеим). Реализация работает на `messages`
(`sender_id` / `recipient_id` / `answer_to`), `message_tags`, `subscriptions`, `attachments`
и `magic_discipline`; слов `category`, `client_msg_id`, `chat_id` в `messages_api/api.php`
по делу нет. То есть это не «устаревшая дока», а **черновик проектирования, который никогда
не был реализован** — он и текстом начинается как переписка («Окей, делаем максимально
простой HTTP-вариант…»). Относиться к нему как к источнику правды нельзя ни в какой части;
сверять с ним код бессмысленно.
