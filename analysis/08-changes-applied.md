...
### Wave 34 — журнал шума и погружение, из которого не уйти молча (2026-09-08)

Обе правки родились из разбора прошлой игры (18–21 сентября 2025), который стал возможен
только потому, что команды терминала зеркалятся в чат мастера. Сам шум статистики не
оставляет: это одно число, которое кроны постоянно стачивают, и через год после игры у всех
участников оно равно нулю.

**Журнал.** Таблица `noise_log`: кто, какая команда, откуда (`app` / `site` / `cms`), чья
доля (`self` / `proxy` / `partner`), сколько просили начислить и сколько реально применилось,
плюс шум до и после и глобальный после. Пишется во всех трёх путях начисления — доли Proxy и
Cross-Link отдельными строками, иначе в статистике деление выглядит как пропавший шум. Запись
идёт тем же соединением и внутри той же транзакции, что и само начисление, поэтому откат
поста на сайте забирает с собой и строку журнала. Ошибка записи журнала игру не роняет —
уходит в error_log.

Заодно журнал закрывает слепое пятно: `SHIFT.PROXY.*`, `USER.*` и `UTILS.*` намеренно **не**
отправляются в чат мастера (`shouldSkipMgNotification`), поэтому по прошлой игре в чате нет ни
одного `PROXY.DEPLOY` — хотя узлы разворачивали трое из четырёх шумомантов, это видно только
по осиротевшим строкам `*_Proxy` в `noisemancy_local`.

**Глубокое погружение.** Разбор показал 19 входов и 2 выхода: механика, которая начисляет шум
именно при возвращении, почти всегда до него не доходила. Три причины и три правки:

- *Игрок не понимал, что нужно число от мастера.* Требование стояло в конце длинной простыни
  текста. Теперь оно наверху и описывает порядок так, как он есть на самом деле: погружение
  идёт своим ходом, а глубину мастер называет **по его итогам** — по тому, что игрок успел
  там сделать, — и уже с этим числом игрок выходит. Первая формулировка звучала как «спроси
  прямо сейчас» и врала про механику; поправлено по замечанию владельца.
- *Погружение можно было просто бросить.* Пока сессия активна, терминал не выполняет ничего,
  кроме `DEEP_DIVE.END`, `HELP` и `UTILS.*` — с напоминанием, как выйти. Это и есть смысл
  механики: из глубины снаружи ничего не сделать. Плюс напоминание при каждом открытии
  терминала, потому что сессия переживает его закрытие.
- *Мастер не знал, что от него ждут.* Раньше ему уходило безликое «Команда в терминале:
  DEEP_DIVE.START», а выход не уходил вовсе. Теперь: «Ушёл в глубокое погружение и ждёт от вас
  глубину (1-5), чтобы вынырнуть» и «Вернулся из погружения с глубины N».

Проверено на эмуляторе целиком: вход → блокировка посторонней команды → напоминание после
перезахода → выход с глубиной 3. В журнале строка `DEEP_DIVE.END`, `requested = 3.0`
(значение от мастера, справочник его не перебивает), `applied = 2.503`; в чате мастера обе
реплики. Тестовые данные и весь журнал очищены — он начинается с чистого листа.

**Про бэкапы этого дня — важное.** Зеркало сайта снималось `wget` в фоне и доехало
только к вечеру, а архив я упаковал раньше, поймав недокачанный срез: в нём 146 файлов
вместо 1886. Он переименован в `ftp-code-20260908-182953-PARTIAL.tar.gz`, полный лежит рядом
как `ftp-code-full-20260908.tar.gz` (32 МБ распакованными, весь `public_html` вместе с
`system/`). У полного есть нюанс: часть файлов докачивалась уже после моих заливок, поэтому
`system/libs/site_noisemancy.php` в нём — промежуточная версия (деление уже есть, журнала
ещё нет). **Для отката пользоваться нужно точечными оригиналами** `*.orig-20260908` — они
сняты непосредственно перед первой правкой и проверены побайтно.

### Wave 35 — хвосты шумомантии закрыты (2026-09-09)

- **Потолок глобального шума выровнен.** `api.php` не ограничивал глобалку `NOISE_GMAX`, а
  `service.php` и `siteNoisemancy::raiseNoise` ограничивали: один и тот же прирост через
  приложение и через сайт давал разный результат, и мобильный путь мог утащить общий шум выше
  десятки. Теперь потолок применяется во всех трёх путях.
- **Цены сайтовых команд переехали в справочник.** `SITE_POST_ANON_NOISE` и
  `SITE_POST_SPOOF_NOISE` были константами в `item_add.php`, плюс третья копия
  (`ARCANEOVERFLOW_POST_SPOOF_NOISE`) в `arcaneoverflow/index.php`. Обе точки теперь спрашивают
  цену у `noise_command_costs` — через `siteNoisemancy::commandCost()` на слое движка и через
  `nm_resolve_command_cost()` на PDO. Константы остались запасным значением на случай, если
  строки в справочнике нет. Весь баланс шума, мобильный и сайтовый, правится из одной таблицы.
- **`CROSS.LINK` перестала врать про цену.** В справочнике у неё стояла 1, а обработчик
  начисление вообще не вызывает — за связывание хранилищ шум не берётся. Цена приведена к 0,
  чтобы `HELP` не показывал игроку то, чего не происходит. Вопрос «должна ли связь стоить
  шума» остаётся к владельцу: если должна, нужен вызов начисления в
  `TerminalProxyCommands.handleCrossLinkCommand`.
- **Пометки для Жени оставлены в его же коде.** Он в `analysis/` не заглядывает, поэтому у
  самих констант (`SITE_POST_ANON_NOISE`, `SITE_POST_SPOOF_NOISE`,
  `ARCANEOVERFLOW_POST_SPOOF_NOISE`) теперь стоит блок: цена уехала в `noise_command_costs`,
  правится в панели, константа осталась запасным значением и менять её бесполезно, пока строка
  в справочнике есть. В docblock `siteNoisemancy::raiseNoise()` — что метод теперь сам делит
  шум на Proxy и Cross-Link, пишет в `noise_log` и берёт цены из справочника.
- **Журнал виден в панели.** На странице «Шум» — последние 40 начислений (игрок, команда,
  источник, чья доля, просили/начислено, что стало) и две сводки за сутки: кто нашумел и чем
  шумели. Раньше `noise_log` можно было прочитать только SQL-запросом.

**A3 закрыт живьём.** `SHIFT.PROXY.DEPLOY` и `CROSS.LINK` прогнаны с эмулятора на боевом
сервере (игра ещё не началась, владелец разрешил). Развёртывание узла создало эффект на 24 часа
и строку `bas_Proxy`, связь — эффекты и метки ауры у обоих участников с правильными текстами.
Главное: **деление заработало ровно как задумано** — следующая команда ценой 2 разложилась на
1.0 узлу Proxy, 0.5 партнёру и 0.5 игроку, и всё это видно в журнале тремя строками с ролями.
Эффекты, метки, узел, шум и журнал после проверки удалены; в ауре `bas` остались две старые
метки «Связь с шумомантом Unknown User» от прошлых тестов — не мои, не трогал.

**Разбор баланса — отдельным документом:** [15-noise-balance.md](15-noise-balance.md). Коротко:
кроны стачивают 1.5 сырых единицы в час, а реальный темп прошлой игры давал 0.4–1.2 — то есть
шум не мог накопиться ни у кого, и эффекты уровней 3–5 не включались ни разу. Ребаланс владелец
будет решать отдельно.

### Wave 36 — баланс шума применён, панель показывает только рабочие кроны (2026-09-09)

Развёрнутый разбор — [15-noise-balance.md](15-noise-balance.md) и
[16-noise-testcases.md](16-noise-testcases.md). Здесь только то, что изменилось в коде.

**Общий файл настроек.** `noize_api/tuning_constants.php` — все `NOISE_*` в одном месте, без
побочных эффектов, подключается из `config.php` (мобильный API), `service.php` (форум) и
`site_noisemancy.php` (CMS). До этого настройки жили в `config.php`, который читает только
мобильный путь, а два других брали зашитые в код умолчания. Совпадали они случайно — любая
правка баланса развела бы пути молча. Проверено: одинаковый вызов через API и через сервис
даёт одно и то же число.

**Сам баланс.** Авторост выключен на обеих шкалах, спад во всех кронах стал пропорциональным
(`новое = текущее − текущее × доля − добавка`), пороги активности опущены с 9 и 36 команд в
час до 1 и 4, софткап отодвинут до 6.0 и сделан положе. После первой итерации верхние уровни
оказались недостижимы (четвёртый требовал ~12 команд в час) — по замечанию владельца спад
ослаблен до −20 %, и теперь пятый уровень берётся рывком, а равновесие остаётся на третьем.

**Панель.** Настройки выключенных кронов (`auto_increase_noise.php`, `auto_decrease_noise.php`)
убраны из формы: владелец проверил планировщик beget — запущены только задания с `_cron` в
имени. Крутить то, что ни на что не влияет, вредно. Сами скрипты на сервере оставлены, они
ещё могут пригодиться, и их значения форма сохраняет нетронутыми — иначе при включении крон
подхватил бы свои внутренние умолчания. Вместо них в форме появились доли спада
(`local_down_pct`, `global_down_pct`), которых там раньше не было вовсе, с проверкой диапазона
0..1 вместо 0..10. Проверено сохранением из живой панели: в `tuning.php` остались все восемь
ключей, скрытые — с прежними значениями.
